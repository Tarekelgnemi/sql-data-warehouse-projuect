...
PRINT '>> Trancating Table: silver.CRM_cost_info';
TRUNCATE TABLE silver.CRM_cost_info;
PRINT '>> Inserting Date into: silver.CRM_cost_info';

INSERT INTO silver.CRM_cost_info (
     cst_id,
	 cst_key,
	 cst_firstname,
	 cst_lastname,
...
SELECT
    ...
    CASE WHEN sls_order_dt = 0 OR LEN(sls_order_dt) != 8 THEN NULL
         ELSE CONVERT(INT,
                 CONVERT(CHAR(8),
                 CAST(CAST(sls_order_dt AS CHAR(8)) AS DATE),112))
    END AS sls_order_dt,

    CASE WHEN sls_ship_dt = 0 OR LEN(sls_ship_dt) != 8 THEN NULL
         ELSE CONVERT(INT,
                 CONVERT(CHAR(8),
                 CAST(CAST(sls_ship_dt AS CHAR(8)) AS DATE),112))
    END AS sls_ship_dt,

    CASE WHEN sls_due_dt = 0 OR LEN(sls_due_dt) != 8 THEN NULL
         ELSE CONVERT(INT,
                 CONVERT(CHAR(8),
                 CAST(CAST(sls_due_dt AS CHAR(8)) AS DATE),112))
    END AS sls_due_dt,

    CASE WHEN sls_sales IS NULL OR sls_sales <=0 OR sls_sales != sls_qyantity * ABS(sls_price)
         THEN sls_qyantity * ABS(sls_price)
         ELSE sls_sales
    END AS sls_sales,
    ...
    CASE WHEN sls_price IS NULL OR sls_price <= 0
         THEN sls_sales / NULLIF(sls_qyantity, 0)
         ELSE sls_price
    END AS sls_price
